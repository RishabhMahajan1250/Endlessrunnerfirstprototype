...
public class obsgen : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i <= 26/1.5f; i++)
        {
           

            GameObject a = Instantiate(obstacle) as GameObject;
            a.transform.position = new Vector3(Random.Range(-2, 4),transform.position.y , transform.position.z + i * 7*1.5f);
            a.transform.localScale = new Vector3(Random.Range(100, 400), a.transform.localScale.y, a.transform.localScale.z);
        }
        for (int i = 0; i <= 13/1.5f; i++)
        {


            GameObject a = Instantiate(obstacleslide) as GameObject;
            a.transform.position = new Vector3(transform.position.x - 2, 1.5f, transform.position.z + i * 14*1.5f);
